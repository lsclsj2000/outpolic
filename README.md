...
- `mapper` : `mapper.xml`에서 실행할 메서드의 인터페이스 정의
- `service` : impl에서 실행할 비즈니스 로직의 메서드 인터페이스 정의
- `impl` : 비즈니스 로직 구현체

**[ resources ]**
- `mapper` : SQL 쿼리 정의
- `static` : 정적 리소스 (CSS, JS 등)
- `templates` : 동적 리소스 (Thymeleaf 템플릿 등)


<br></br>
## 7. 주요 기능 설명
### ① 사용자 기능
- **외주 등록** : 공급자의 외주 등록 및 수정/공유 기능
- **포트폴리오 등록** : 공급자의 포트폴리오 등록 및 수정/공유 기능
- **외주 신청/문의** : 공급자의 외주/포트폴리오에 수요자가 신청 또는 문의하여 외주를 의뢰하는 기능
- **외주 진행** : 승인된 외주 신청/문의에 대하여 각 단계별로 외주를 진행하며 보고 및 피드백을 통해 효율적인 관리를 제공하는 기능

### ② 관리자 기능
- **회원 관리** : 기업 및 일반 회원의 공급/수요 권한 관리
- **외주 관리** : 외주글 수정 및 진행 관리
- **포트폴리오 관리** : 포트폴리오 수정 및 상태 관리
...
|------|-----------|
| 이성재 | 추후 추가 |
| 김주완 | User <br>1. 포폴 문의<br>-- 포폴 문의 페이지<br>-- 보낸 포폴 문의 목록 페이지<br>-- 보낸 포폴 문의 목록 상세 페이지<br>-- 받은 포폴 문의 목록 페이지<br>-- 받은 포폴 문의 목록 상세페이지<br><br>Enter<br>1. 포트폴리오 등록 페이지<br>-- 포트폴리오 등록 페이지<br>-- 포트폴리오 삭제<br>-- 포트폴리오 수정 페이지<br>-- 내 포트폴리오 페이지<br><br>2. 외주 목록 페이지<br>-- 내 외주 목록 페이지<br>-- 외주 등록 페이지 (기업 로그인 후 사용)<br>-- 외주 삭제<br>-- 외주 수정 페이지 (기업 로그인 후 사용)<br><br>3. 외주 신청 페이지<br>-- 외주 신청 페이지<br>-- 보낸 외주 신청 목록 페이지 (기업 로그인 후 사용)<br>-- 보낸 외주 신청 목록 상세 페이지<br>-- 받은 외주 신청 목록 페이지 (기업 로그인 후 사용)<br>-- 받은 외주 신청 목록 상세페이지<br> |
| 김한별 | User<br>1. 컨텐츠<br>-- 컨텐츠 목록 뷰<br>-- 카테고리 목록 뷰<br>-- 컨텐츠 상세페이지<br>-- 상세페이지 조회시 조회수 증가<br><br>2. 메인 페이지<br>-- 카테고리<br>-- 인기있는 외주<br>-- 인기있는 포트폴리오<br>-- 최근에 달린 리뷰<br><br>3. 모달창<br>-- 포트폴리오를 누르면 나오는 상세페이지를 모달창으로 구성<br><br>4. 리뷰<br>-- 리뷰 모달창 가능<br><br>5. 즐겨찾기<br>-- 로그인 후 콘텐츠 즐겨찾기 기능<br><br>6. 검색<br>--검색 키워드로 콘텐츠 정렬<br><br>7. 기업 페이지<br>-- 기업의 상세페이지<br><br>Enter<br>1. 컨텐츠<br>-- 컨텐츠 목록 뷰<br>-- 카테고리 목록 뷰<br>-- 컨텐츠 상세페이지<br>-- 상세페이지 조회시 조회수 증가<br><br>2. 메인 페이지<br>-- 카테고리<br>-- 인기있는 외주<br>-- 인기있는 포트폴리오<br>-- 최근에 달린 리뷰<br><br>3. 모달창<br>-- 포트폴리오를 누르면 나오는 상세페이지를 모달창으로 구성<br><br>4. 리뷰<br>-- 리뷰 모달창 가능<br><br>5. 즐겨찾기<br>-- 로그인 후 콘텐츠 즐겨찾기 기능<br><br>6. 검색<br>--검색 키워드로 콘텐츠 정렬<br><br>7. 기업 페이지<br>-- 기업의 상세페이지<br><br>Admin<br>1. 통계<br>-- 랭킹통계<br>-- 조회수 통계<br>-- 검색어 통계<br><br>2. 카테고리<br>-- 카테고리 수정<br>-- 카테고리 삭제<br>-- 카테고리 생성<br><br>3. 대쉬보드<br>-- 전체적인 수익,회원수,회원가입 등등 통계 |
| 신세영 | 공통</br>1. 인터셉터 작업</br></br>2. 로그인 기능</br></br>3. 로그아웃 기능</br></br>4. 북마크 페이지 기능 </br></br>User</br>1. 회원가입</br>-- 일반 유저 회원가입</br>-- 기업 회원으로 전환</br></br>2. 비밀번호 변경</br></br>3. 회원 탈퇴</br></br>4. 일반 유저 마이페이지</br>-- 마이페이지 메인(전체)</br>-- 마이페이지 개인정보 수정</br>-- 마이페이지 프로필 사진 수정</br></br>Enter</br>1. 기업 유저 마이페이지</br>-- 마이페이지 메인(전체)</br>-- 마이페이지 기업정보 수정</br>-- 마이페이지 개인 정보 수정</br></br>Admin</br>1. 관리자 로그인</br></br>2. 회원 리스트</br>-- 회원 목록 조회</br>-- 회원 검색/정렬</br>-- 회원 정보 수정</br></br>3. 기업 리스트</br>-- 기업 목록 조회</br>-- 기업 검색/정렬</br>-- 기업 정보 수정</br></br>4. 로그인 기록 확인</br>-- 로그인 기록 검색</br>-- 최근 로그인 수 그래프 조회</br></br>5. 관리자 권한 설정</br>-- 관리자 권한 조회/수정</br></br> |
| 유사랑 | User<br>1. 문의<br>-- 전체 게시물<br>-- 공지사항<br>-- 문의 내역<br>-- FAQ<br>-- 문의 작성<br>-- 문의 상세 내역<br><br>2. 신고 작성<br><br>3. 채팅(모달창)<br>-- 프로필<br>-- 채팅방<br><br>Enter<br>1. 문의<br>-- 전체 게시물<br>-- 공지사항<br>-- 문의 내역<br>-- FAQ<br>-- 문의 작성<br>-- 문의 상세 내역<br><br>2. 신고 작성<br><br>3. 채팅(모달창)<br>-- 프로필<br>-- 채팅방<br><br>4. 외주 진행<br>-- 내 외주 진행 목록<br>-- 진행 외주 상세 페이지<br><br>Admin<br>1. 문의<br>-- 문의 내역<br>-- 문의 자원 등록<br><br>2. 신고<br>-- 신고 내역<br>-- 신고 자원 등록<br><br>3. 제재<br>-- 제재 내역<br>-- 제재 처리<br>-- 권한 설정<br>-- 제재 자원 등록<br><br>4. 외주<br>-- 외주 진행 현황<br>-- 외주 채팅방 관리 |
...
| 페이지 | 주요 기능 |
|--------|----------|
| 메인 | 로그인, 마이페이지, 검색, 메뉴바, 카테고리,  기업 전환, 인기있는 외주, 인기있는 포트폴리오, 최근 작성된 리뷰 |
| 마이페이지 | 내 정보 관리, 내가 작성한 리뷰, 결제 내역, 문의 내역, 회원 탈퇴 |
